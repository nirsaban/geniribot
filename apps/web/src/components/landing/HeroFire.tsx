...
export function HeroFire() {
  ...
  return (
    <section className="relative isolate overflow-hidden border-b border-white/5 bg-[#07050a]">
      {/* ambient ember glow (also the no-WebGL fallback) */}
      <div className="pointer-events-none absolute inset-x-0 bottom-0 -z-20 h-2/3 bg-[radial-gradient(ellipse_70%_100%_at_50%_100%,rgba(255,120,20,0.28),transparent_70%)]" />

      {/* the fire */}
      <FireCanvas className="absolute inset-0 -z-10 h-full w-full" />

      {/* keep text legible over the flames */}
      <div className="pointer-events-none absolute inset-0 -z-10 bg-gradient-to-t from-[#07050a] via-[#07050a]/55 to-[#07050a]/85" />

      <div className="mx-auto max-w-6xl px-5 pb-28 pt-20 text-center md:pb-36 md:pt-28">
        <span className="inline-flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.22em] text-amber-300/90">
        ...
        <h1 className="animate-fade-up mt-6 text-[2.7rem] font-black leading-[0.98] tracking-tight text-white sm:text-6xl md:text-[4.3rem]">
          {h.title}
          <br />
          <span className="bg-gradient-to-t from-amber-500 via-orange-400 to-yellow-200 bg-clip-text text-transparent drop-shadow-[0_0_28px_rgba(255,140,30,0.45)]">
            {h.titleAccent}
          </span>
        </h1>
        ...
        <div className="animate-fade-up mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row [animation-delay:0.15s]">
          <Link
            href="/register"
            className="inline-flex items-center justify-center gap-2 rounded-lg bg-gradient-to-b from-amber-400 to-orange-500 px-8 py-3.5 text-base font-bold text-[#1a0a02] shadow-[0_0_50px_-10px_rgba(255,150,40,0.95)] transition hover:from-amber-300 hover:to-orange-400"
          >
            {h.ctaPrimary}
            <svg
              viewBox="0 0 24 24"
          ...
          <a
            href="#pricing"
            className="inline-flex items-center justify-center rounded-lg border border-amber-200/20 px-8 py-3.5 text-base font-semibold text-white transition hover:border-amber-200/40 hover:bg-white/5"
          >
            {h.ctaSecondary}
          </a>
        </div>
...
function FireCanvas({ className = "" }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const fragSrc = `
      ...
      // black -> ember -> orange -> gold -> white
      vec3 heatRamp(float h) {
        vec3 c = mix(vec3(0.03, 0.005, 0.0), vec3(0.55, 0.06, 0.01), smoothstep(0.00, 0.32, h));
        c = mix(c, vec3(1.00, 0.36, 0.04), smoothstep(0.28, 0.58, h));
        c = mix(c, vec3(1.00, 0.78, 0.22), smoothstep(0.55, 0.82, h));
        c = mix(c, vec3(1.00, 0.97, 0.88), smoothstep(0.84, 1.00, h));
        return c;
      }
      ...
      void main() {
        ...
        col += vec3(1.0, 0.72, 0.32) * emb * 1.4;
