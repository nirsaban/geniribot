...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const canManageTeam = hasRole(session.role as Role, "ADMIN");
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-l border-line bg-white md:flex">
        ...
        <div className="border-t border-line p-3">
          {session.sa && (
            <Link
              href="/admin"
              className="mb-1 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-violet-700 hover:bg-violet-50"
            >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* Mobile top bar — the drawer is the only navigation below md. */}
        <header className="sticky top-0 z-40 flex items-center justify-between border-b border-line bg-white px-4 py-3 md:hidden">
          <span className="font-extrabold text-ink">{he.appName}</span>
          <MobileNav orgName={orgName} canManageTeam={canManageTeam} footer={logoutButton} />
        </header>
